...
const ChatPage: React.FC<ChatPageProps> = ({ onBack }) => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Uses the original /chat endpoint solely for ingestion
      ...
      if (response.ok) {
        ...
        setUploadStatus({ type: 'success', message: 'Document successfully analyzed.' });
        const systemMessage: Message = {
          id: Date.now().toString(),
          type: 'assistant',
          content: result.answer || `Document "${file.name}" uploaded and processed. Ready for your questions!`,
          timestamp: new Date()
        };
        // Clear previous messages on new document upload to avoid cross-contamination
      ...
      setUploadStatus({ type: 'error', message: 'Failed to upload and process the document.' });
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Use the JSON API endpoint and bypass sending the file every time
      const requestBody = {
        message: userMessage.content,
        session_id: uploadedFile.name // Using filename as simple session ID for context tracking
      };
      ...
      if (response.ok) {
        ...
        const sources = result.context_docs?.map((doc: any) => ({
          source: doc.metadata?.document_name || doc.metadata?.source || 'Unknown',
          page_number: doc.metadata?.page_number || 1
        })) || [];
        
        // Remove duplicates
        const uniqueSources = sources.filter((v: Source, i: number, a: Source[]) => 
          a.findIndex(t => (t.source === v.source && t.page_number === v.page_number)) === i
        );
      ...
      <div className="w-full max-w-4xl border border-gray-700 rounded-2xl overflow-hidden shadow-lg bg-black relative">
                      ...
                      <div className="text-sm text-gray-200 space-y-1">
                          ...
                          <div className="mt-3 pt-2 border-t border-gray-700/50">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {message.sources.map((s, idx) => (
                                <span key={idx} className="text-[10px] bg-cyan-900/40 text-cyan-200 px-2 py-1 rounded border border-cyan-800/50">
                                  {s.source} - Page {s.page_number}
                                </span>
                              ))}
                            </div>
                          </div>
                        ...
                        {message.metrics && (
                          <div className="mt-2 text-[10px] text-gray-500 font-mono">
                            Retrieval: {message.metrics.retrieval_ms?.toFixed(0) || 0}ms | Total: {message.metrics.total_ms?.toFixed(0) || 0}ms
                          </div>
                        )}
                      </div>
                    ) : (
        ...
        {uploadedFile && (
          <div className="border-t border-gray-800/50 p-4 bg-black/60 backdrop-blur-xl">
            {uploadStatus && (
              <div className={`mb-3 text-sm px-3 py-2 rounded-lg border ${uploadStatus.type === 'error' ? 'bg-red-900/20 border-red-500/50 text-red-200' : 'bg-green-900/20 border-green-500/50 text-green-200'}`}>
